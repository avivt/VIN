...
class VinBlock(object):
    ...
    def __init__(self, in_x, in_s1, in_s2, in_x_channels, imsize, batchsize=128,
        ...
        self.w_list = [init_weights_T(l_q, 1, 3, 3) for i in range(0,k+1)] # 90 parameters
        # feedback weights from v layer into q layer (~transition probabilities in Bellman equation)
        self.w_fb_list = [init_weights_T(l_q, 1, 3, 3) for i in range(0,k)] # 90 parameters

        self.q = conv2D_keep_shape(self.r, self.w_list[0], image_shape=[batchsize, self.w1.shape.eval()[0],
                                                                imsize[0], imsize[1]],
                                   filter_shape=self.w_list[0].shape.eval())
        self.v = T.max(self.q, axis=1, keepdims=True)

        for i in range(0, k-1):
            self.q = conv2D_keep_shape(T.concatenate([self.r, self.v], axis=1), T.concatenate([self.w_list[i+1], self.w_fb_list[i]],
                                                                                              axis=1),
                                       image_shape=[batchsize, self.w1.shape.eval()[0]+1, imsize[0], imsize[1]],
                                       filter_shape=T.concatenate([self.w_list[i+1], self.w_fb_list[i]], axis=1).shape.eval())
            self.v = T.max(self.q, axis=1, keepdims=True)

        # do one last convolution
        self.q = conv2D_keep_shape(T.concatenate([self.r, self.v], axis=1), T.concatenate([self.w_list[k], self.w_fb_list[k-1]], axis=1),
                                   image_shape=[batchsize, self.w1.shape.eval()[0]+1, imsize[0], imsize[1]],
                                   filter_shape=T.concatenate([self.w_list[k], self.w_fb_list[k-1]], axis=1).shape.eval())
        ...
        self.params = self.w_list + self.w_fb_list + [self.w0, self.bias, self.w1, self.w_o]
